...
chd_status_t chd_nn_model_load(const char *model_path,
    ...
    if (model_path == nullptr || out == nullptr) {
        chd::detail::set_last_error("chd_nn_model_load: null argument");
    ...
    *out = nullptr;
    ...
    try {
        ...
        *out = handle.release();
        ...
        chd::detail::set_last_error(std::string("chd_nn_model_load: ") + e.what());
...
chd_status_t chd_nn_model_load(const char *, const chd_nn_session_opts_t *,
                                chd_nn_model_t **) {
    return nn_disabled("chd_nn_model_load");
}
