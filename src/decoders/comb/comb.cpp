...
Comb::FrameBuffer::FrameBuffer(const chd::metadata::LdDecodeMetaData::VideoParameters &videoParameters_,
...
void Comb::FrameBuffer::loadFields(const chd::decoders::SourceField &firstField, const chd::decoders::SourceField &secondField)
{
    // Interlace the input fields and place in the frame buffer
    int32_t fieldLine = 0;
    rawbuffer.clear();
    for (int32_t frameLine = 0; frameLine < frameHeight; frameLine += 2) {
        rawbuffer.insert(rawbuffer.end(),
                         firstField.data.begin() + (fieldLine * videoParameters.fieldWidth),
                         firstField.data.begin() + ((fieldLine + 1) * videoParameters.fieldWidth));
        rawbuffer.insert(rawbuffer.end(),
                         secondField.data.begin() + (fieldLine * videoParameters.fieldWidth),
                         secondField.data.begin() + ((fieldLine + 1) * videoParameters.fieldWidth));
        fieldLine++;
    }
...
void Comb::FrameBuffer::doCNR()
{
    ...
    auto iFilter(chd::decoders::filter::f_nrc);
    auto qFilter(chd::decoders::filter::f_nrc);

    // Filter delay (since it's a symmetric FIR filter)
    const int32_t delay = chd::decoders::filter::c_nrc_b.size() / 2;
...
void Comb::FrameBuffer::doYNR()
{
    ...
    auto yFilter(chd::decoders::filter::f_nr);

    // Filter delay (since it's a symmetric FIR filter)
    const int32_t delay = chd::decoders::filter::c_nr_b.size() / 2;
