...
void TransformPal::updateConfiguration(const chd::metadata::LdDecodeMetaData::VideoParameters &_videoParameters,
                                       double threshold, const std::vector<double> &_thresholds)
{
    ...
    if (_thresholds.size() == 0) {
        ...
        thresholds.assign(thresholdsSize, threshold * threshold);
    } else {
        // Square the provided thresholds
        assert(_thresholds.size() == static_cast<size_t>(thresholdsSize));
