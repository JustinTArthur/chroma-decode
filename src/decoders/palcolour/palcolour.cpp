...
template <typename T>
T qBound(T lo, T v, T hi) { return std::max(lo, std::min(v, hi)); }
}
...
void PalColour::updateConfiguration(const chd::metadata::LdDecodeMetaData::VideoParameters &_videoParameters,
...
void PalColour::buildLookUpTables()
{
    ...
    if(videoParameters.fieldHeight != 263) {
        for (int32_t i = 0; i < videoParameters.fieldWidth; i++) {
...
void PalColour::decodeField(const chd::decoders::SourceField &inputField, const double *chromaData, chd::output::ComponentFrame &componentFrame)
{
...
void PalColour::doYNR(double *Yline)
{
    ...
    auto yFilter(chd::decoders::filter::f_nrpal);

    // Filter delay (since it's a symmetric FIR filter)
    const int32_t delay = chd::decoders::filter::c_nrpal_b.size() / 2;
...
void PalColour::decodeLine(const chd::decoders::SourceField &inputField, const ChromaSample *chromaData, const LineInfo &line,
                           chd::output::ComponentFrame &componentFrame)
{
    ...
    if (endPos < videoParameters.activeVideoEnd) {
        chd::log::debug() << "Warning: Tried to decode video outside max width!";
    }
    ...
    if (PREFILTERED_CHROMA && configuration.simplePAL) {
        ...
        static constexpr auto palUvFilter = chd::decoders::filter::makeFIRFilter(palUvFilterCoeffs);
        ...
        static constexpr auto palMUvFilter = chd::decoders::filter::makeFIRFilter(palMUvFilterCoeffs);

        const auto& uvFilter = (videoParameters.system == chd::metadata::PAL) ? palUvFilter : palMUvFilter;

        const int32_t overlap = palUvFilterCoeffs.size() / 2;
        const int32_t startPos = videoParameters.activeVideoStart - overlap;
        const int32_t endPos = videoParameters.activeVideoEnd + overlap + 1;
        ...
        for (int32_t i = videoParameters.activeVideoStart; i < endPos; i++) {
            pv[i] = pu[i];
            qv[i] = qu[i];
        }
    } else {
    ...
    const uint16_t *comp = inputField.data.data() + (line.number * videoParameters.fieldWidth);
