...
void TransformPal2D::filterFields(const std::vector<chd::decoders::SourceField> &inputFields, int32_t startIndex, int32_t endIndex,
                                  std::vector<const double *> &outputFields)
{
    ...
    assert(outputFields.size() == static_cast<size_t>(endIndex - startIndex));

    // Allocate and clear output buffers
    chromaBuf.resize(endIndex - startIndex);
    for (size_t i = 0; i < chromaBuf.size(); i++) {
        chromaBuf[i].assign(videoParameters.fieldWidth * videoParameters.fieldHeight, 0.0);
...
void TransformPal2D::filterField(const chd::decoders::SourceField& inputField, int32_t outputIndex)
{
...
void TransformPal2D::forwardFFTTile(int32_t tileX, int32_t tileY, int32_t startY, int32_t endY, const chd::decoders::SourceField &inputField)
{
                                     ...
                                     chd::output::ComponentFrame &componentFrame)
{
    ...
    const chd::decoders::SourceField &inputField = inputFields[fieldIndex];
    const int32_t firstFieldLine = inputField.getFirstActiveLine(videoParameters);
    const int32_t lastFieldLine = inputField.getLastActiveLine(videoParameters);
